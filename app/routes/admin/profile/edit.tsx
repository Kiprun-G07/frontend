...
export default function EditAdminProfile() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Load admin profile being edited
        const profileRes = await apiFetch(`/api/admin/profile/${adminId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch(`/api/admin/profile/${adminId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(profile),
      });
      
      // Return to dashboard after editing other admin
  ...
  return (
    <main className="max-w-2xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl">Edit Admin: {profile.name}</h1>
        <button 
          type="button"
